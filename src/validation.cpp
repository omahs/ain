...
std::vector<CAuctionBatch> CollectAuctionBatches(const CVaultValueSheet& collLoan, const TAmounts& collBalances, const TAmounts& loanBalances)
{
...
void CChainState::ProcessLoanEvents(const CBlockIndex* pindex, CCustomCSView& cache, const CChainParams& chainparams)
{
    ...
    if (pindex->nHeight % chainparams.GetConsensus().blocksCollateralizationRatioCalculation() == 0) {
        ...
        cache.ForEachVaultCollateral([&](const CVaultId& vaultId, const CBalances& collaterals) {
            auto vaultValues = cache.GetVaultValueSheet(vaultId, collaterals, pindex->nHeight, pindex->nTime,
            ...
            assert(scheme);
            if (scheme->ratio <= vaultValues.val->ratio()) {
            ...
            for (auto it = loanTokens->balances.begin(); it != loanTokens->balances.end();) {
                ...
                if (subInterest > 0) {
                    ...
                    cache.ReduceInterest(pindex->nHeight, vaultId, vault->schemeId, tokenId, tokenValue, subInterest);
                } else {
            ...
            auto batches = CollectAuctionBatches(*vaultValues.val, collaterals.balances, loanTokens->balances);
            ...
            if (pvaultHistoryDB) {
                pvaultHistoryDB->WriteVaultState(cache, *pindex, vaultId, vaultValues.val->ratio());
            }
        ...
        for (uint32_t i = 0; i < data.batchCount; i++) {
            ...
            if (auto bid = view.GetAuctionBid({vaultId, i})) {
                ...
                if (auto token = view.GetLoanTokenByID(batch->loanAmount.nTokenId)) {
                    view.IncreaseInterest(pindex->nHeight, vaultId, vault->schemeId, batch->loanAmount.nTokenId,
                                          token->interest, batch->loanAmount.nValue);
                }
                for (const auto& col : batch->collaterals.balances) {
                    auto tokenId = col.first;
                    auto tokenAmount = col.second;
    ...
    for (auto& [vaultId, rate, schemeId] : loanInterestRates) {
        ...
        if (LogAcceptCategory(BCLog::TOKENSPLIT)) {
            LogPrint(BCLog::TOKENSPLIT, "TokenSplit: V Interest (%s: %s => %s, %s => %s)\n",
                ...
                GetInterestPerBlockHighPrecisionString({newRateToHeight, oldRateToHeight.negative}),
