...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-consolidaterewards=<token-symbol>", "Consolidate rewards on startup. Accepted multiple times for each token symbol", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-consolidaterewards")) {
        ...
        auto fullRewardConsolidation = true;
        for (const auto& tokenSymbol : tokenSymbolArgs) {
            LogPrintf("Consolidate rewards for token: %s\n", tokenSymbol);
            auto token = pcustomcsview->GetToken(tokenSymbol);
            if (!token) continue;
            fullRewardConsolidation = false;
            ...
            auto getIterValueFn = [it = std::ref(it)] { return it.get().Key().owner; };

            ConsolidateRewards(*pcustomcsview, ::ChainActive().Height(), 
