...
extern std::vector<CAuctionBatch> CollectAuctionBatches(const CVaultValueSheet& collLoan, const TAmounts& collBalances, const TAmounts& loanBalances);
...
BOOST_AUTO_TEST_CASE(high_precision_interest_rate_to_string_tests)
{
    ...
    auto res = GetInterestPerBlockHighPrecisionString({1, true});
    BOOST_CHECK_EQUAL(res, "-0.000000000000000000000001");
    res = GetInterestPerBlockHighPrecisionString({std::numeric_limits<int64_t>::min(), true});
...
BOOST_AUTO_TEST_CASE(loan_interest_rate)
{
    ...
    BOOST_REQUIRE(mnview.ReduceInterest(6, vault_id, id, token_id, 1 * COIN,
                                        (interestToHeight.amount + interestPerBlock.amount).GetLow64()));
    rate = mnview.GetInterestRate(vault_id, token_id, 6);

    BOOST_REQUIRE(rate);
    BOOST_CHECK_EQUAL(rate->interestToHeight.amount.GetLow64(), 0);

    BOOST_REQUIRE(mnview.ReduceInterest(6, vault_id, id, token_id, 1 * COIN, 0));
...
BOOST_AUTO_TEST_CASE(collateralization_ratio)
{
    ...
    auto vaultValues = mnview.GetVaultValueSheet(vault_id, *collaterals, 10, 0);
    BOOST_REQUIRE(vaultValues.ok);
    BOOST_CHECK_EQUAL(vaultValues.val->ratio(), 78);
}

BOOST_AUTO_TEST_CASE(auction_batch_creator)
{
    {
        CVaultValueSheet collLoan = {
            7000 * COIN, 1000 * COIN,
